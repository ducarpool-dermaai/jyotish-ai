...
async function callClaude(messages, max_tokens=1000){
  const res=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens,messages})});
...
// ─── EMPTY STATE ─────────────────────────────────────────────────────────────
const NeedChart = ({label}) => (
  <div className="empty">
    <span className="eicon">⬡</span>
    Fill in your birth details above and click <strong>✦ Cast Kundali</strong> to unlock {label}.
  </div>
);

// ─── EMPTY STATE ─────────────────────────────────────────────────────────────
function NeedChart({label}){
  return(
    <div style={{textAlign:'center',padding:'44px 18px',fontFamily:'var(--ff-l)',fontSize:16,color:'var(--muted)',fontStyle:'italic',lineHeight:1.9}}>
      <span style={{fontSize:28,display:'block',marginBottom:10,opacity:.35}}>⬡</span>
      Fill in your birth details above and click <strong style={{fontFamily:'var(--ff-h)',letterSpacing:'.1em'}}>Cast Kundali</strong> to unlock {label}.
    </div>
  );
}
...
{/* Snapshot */}
          ...
          <div className="card">
            <div className="tabs no-print">
              {[['form','📋 Form'],['chart','⬡ Kundali'],['planets','☉ Planets'],['panchang','📅 Panchang'],['dasha','⌛ Dasha'],['doshas','⚠ Doshas'],['transits','🔭 Transits'],['chat','💬 Chat'],['match','💍 Match'],['remedies','🙏 Remedies'],['reading','✦ Reading']].map(([id,lb])=>(
            ...
            {tab==='form'&&(
              <div>
                <div className="fg2">
                  <div className="fg"><label className="flbl">Full Name</label><input className="fi" placeholder="e.g. Aditya Sharma" value={f.name} onChange={e=>set('name',e.target.value)}/></div>
                  <div className="fg"><label className="flbl">Date of Birth (DD MM YYYY)</label><div className="frow"><input className="fi" placeholder="DD" maxLength={2} value={f.day} onChange={e=>set('day',e.target.value)} style={{width:52}}/><input className="fi" placeholder="MM" maxLength={2} value={f.month} onChange={e=>set('month',e.target.value)} style={{width:52}}/><input className="fi" placeholder="YYYY" maxLength={4} value={f.year} onChange={e=>set('year',e.target.value)} style={{width:78}}/></div></div>
                  <div className="fg"><label className="flbl">Time (HH MM, 24h)</label><div className="frow"><input className="fi" placeholder="HH" maxLength={2} value={f.hour} onChange={e=>set('hour',e.target.value)} style={{width:64}}/><input className="fi" placeholder="MM" maxLength={2} value={f.min} onChange={e=>set('min',e.target.value)} style={{width:64}}/></div></div>
                  <div className="fg"><label className="flbl">UTC Offset (IST = 5.5)</label><input className="fi" placeholder="5.5" value={f.tz} onChange={e=>set('tz',e.target.value)}/></div>
                  <div className="fg full"><label className="flbl">Place of Birth</label>
                    <div className="locrow"><div className="fg"><input className="fi" placeholder="e.g. New Delhi, India" value={f.place} onChange={e=>set('place',e.target.value)} onKeyDown={e=>e.key==='Enter'&&geocode(f.place,(la,lo)=>sf(p=>({...p,lat:la,lon:lo})),setGeoLoad,setGeoErr)}/></div>
                    <button className="bs" disabled={geoLoad||!f.place} onClick={()=>geocode(f.place,(la,lo)=>sf(p=>({...p,lat:la,lon:lo})),setGeoLoad,setGeoErr)}>{geoLoad?'Finding…':'🌐 Locate'}</button></div>
                    {geoErr?<p className="fhelp" style={{color:'var(--red)'}}>⚠ {geoErr}</p>:<p className="fhelp">150+ cities built-in · Or enter lat/lon below</p>}
                    {f.lat&&f.lon&&<p className="fhelp" style={{color:'var(--gold)'}}>✓ {f.lat}°N, {f.lon}°E</p>}
                  </div>
                  <div className="fg"><label className="flbl">Latitude</label><input className="fi" placeholder="28.6517" value={f.lat} onChange={e=>set('lat',e.target.value)}/></div>
                  <div className="fg"><label className="flbl">Longitude</label><input className="fi" placeholder="77.2219" value={f.lon} onChange={e=>set('lon',e.target.value)}/></div>
                </div>
                {err&&<div className="err">⚠ {err}</div>}
                <div className="acts"><button className="bp" onClick={castChart}>✦ Cast Kundali</button></div>
              </div>
            )}

            {/* ── Kundali ── */}
            {tab==='chart'&&(chart ? <KundaliChart chart={chart}/> : <NeedChart label='your Kundali chart'/>)}

            {/* ── Planets ── */}
            {tab==='planets'&&(!chart ? <NeedChart label='the planet table'/> : <table className="ptbl">
            ...
            {/* ── Panchang ── */}
            {tab==='panchang'&&(!panch ? <NeedChart label='Panchang details'/> : (
              <div>
            ...
            {/* ── Dasha ── */}
            {tab==='dasha'&&(!dasha ? <NeedChart label='Dasha periods'/> : (
              <div>
            ...
            {/* ── Doshas ── */}
            {tab==='doshas'&&(!doshas ? <NeedChart label='Dosha analysis'/> : (
              <div>
            ...
            {/* ── Today's Transits ── */}
            {tab==='transits'&&(!transits ? <NeedChart label='transit analysis'/> : (
              <div>
